...
export default function WoolCardedSaleList({ items, paging }: Props) {
    ...
    const handlePageChange = useCallback((page: number) => {
        ...
        router.push(`${ROUTES.SALE.WOOL_CARDED}?${params.toString()}`);
    }, [router, searchParams]);
        ...
        return (
            <div className="bg-zinc-800/80 backdrop-blur-md backdrop-saturate-150 rounded-xl border border-zinc-700/50 p-6 text-center py-16">
                <h2 className="text-2xl font-bold text-zinc-200 mb-2">
                    Ingen kartet uld matcher din søgning
                </h2>
                ...
                <Button
                    variant="primary"
                    onPress={() => router.push(ROUTES.SALE.WOOL_CARDED)}
                >
                    Vis al kartet uld
                </Button>
            </div>
