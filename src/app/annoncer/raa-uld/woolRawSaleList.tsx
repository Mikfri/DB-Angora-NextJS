...
export default function WoolRawSaleList({ items, paging }: Props) {
    ...
    const handlePageChange = useCallback((page: number) => {
        ...
        router.push(`${ROUTES.SALE.WOOL_RAW}?${params.toString()}`);
    }, [router, searchParams]);
        ...
        return (
            <div className="bg-zinc-800/80 backdrop-blur-md backdrop-saturate-150 rounded-xl border border-zinc-700/50 p-6 text-center py-16">
                <h2 className="text-2xl font-bold text-zinc-200 mb-2">
                    Ingen rå uld matcher din søgning
                </h2>
                ...
                <Button
                    variant="primary"
                    onPress={() => router.push(ROUTES.SALE.WOOL_RAW)}
                >
                    Vis al rå uld
                </Button>
            </div>
